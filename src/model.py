...
class UKGE_rect_TF(TFParts):
    ...
    def define_main_loss(self): 
        ...
        self._f_prob_hn = f_prob_hn = self.w * (tf.reduce_sum(tf.multiply(self._neg_rel_hn_batch, tf.multiply(self._neg_hn_con_batch, self._neg_t_con_batch)), 2)) + self.b
        self._f_score_hn = f_score_hn = tf.reduce_mean(tf.square(f_prob_hn), 1)

        self._f_prob_tn = f_prob_tn = self.w * (tf.reduce_sum(tf.multiply(self._neg_rel_tn_batch, tf.multiply(self._neg_h_con_batch, self._neg_tn_con_batch)), 2)) + self.b
        self._f_score_tn = f_score_tn = tf.reduce_mean(tf.square(f_prob_tn), 1)
        ...
        self._regularizer = regularizer = tf.add(tf.add(tf.divide(tf.nn.l2_loss(self._h_batch), self.batch_size), tf.divide(tf.nn.l2_loss(self._t_batch), self.batch_size)), tf.divide(tf.nn.l2_loss(self._r_batch), self.batch_size)) # L2 regularizer
        self.main_loss = tf.add(this_loss, self.reg_scale * regularizer)

    # Override abstract method
    def define_psl_loss(self): 
        self.psl_prob = self.w * tf.reduce_sum(tf.multiply(self._soft_r_batch, tf.multiply(self._soft_h_batch, self._soft_t_batch)), 1) + self.b
        self.compute_psl_loss()
